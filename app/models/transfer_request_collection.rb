...
class TransferRequestCollection < ApplicationRecord
  ...
  def transfer_requests_io=(parameters)
    ...
    updated_attributes = parameters.map do |parameter|
      parameter['asset_id'] = uuid_cache[[parameter.delete('source_asset'), Receptacle.base_class.name]] if parameter['source_asset'].is_a?(String)
      parameter['asset'] = parameter.delete('source_asset') if parameter['source_asset'].present?
      parameter['target_asset_id'] = uuid_cache[[parameter.delete('target_asset'), Receptacle.base_class.name]] if parameter['target_asset'].is_a?(String)
