module SampleManifest::PlateBehaviour
  ...
  class Core
    ...
    def io_samples
      samples.map do |sample|
        ...
        ::Io::SampleManifest::SampleMapper.new(
          sample,
          :barcode => container.holder.sanger_human_barcode,
    ...
    def print_labels(&block)
      plates              = self.samples.map { |s| s.assets.first.holder }.uniq
  ...
  def details(&block)
    samples.each do |sample|
      well = sample.assets.first(:include => [ :holder, :map ])
      yield({
        :barcode   => well.holder.sanger_human_barcode,
  ...
  def generate_plates
    ...
    plates = (0...self.count).map do |_|
      Plate.create_plate_with_barcode.tap do |plate|
  ...
  def generate_wells(well_data, plate_id, study_id)
    ...
    well_data.each do |position,sanger_sample_id|
      ...
      wells_to_create << Well.new(:holder => plate, :map => map, :sample => sample)
      study_samples_data << [study_id, sample.id]
    end
    delayed_generate_study_samples(study_samples_data)
    Well.import wells_to_create
    plate.save
    plate.reload
