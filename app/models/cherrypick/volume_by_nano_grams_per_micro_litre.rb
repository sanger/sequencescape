...
module Cherrypick::VolumeByNanoGramsPerMicroLitre
  ...
  def volume_to_cherrypick_by_nano_grams_per_micro_litre(
    ...
    if well_attribute.current_volume < robot_minimum_picking_vol
      well_attribute.current_volume = robot_minimum_picking_vol
    end

    if source_volume < robot_minimum_pick_vol
      warn "Warning: Source volume (#{source_volume}) is less than the robot 
      minimum picking volume (#{robot_minimum_pick_vol})."
    end
    ...
    if source_volume_to_tell_robot_to_pick >= robot_minimum_pick_vol
