...
4. Failed statuses are retained for user reference.
5. If an accession fails three times, the status is updated to 'aborted' indicating that it will not be retried again.
6. If the accessioning process succeeds, the accession number would be updated on the sample (metadata) itself and all status records for that sample deleted.
