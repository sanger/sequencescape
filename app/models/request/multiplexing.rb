class Request::Multiplexing < CustomerRequest # rubocop:todo Style/Documentation
  ...
  redefine_aasm column: :state, whiny_persistence: true do
    ...
    event :pass   do transitions to: :passed,      from: %i[pending started] end
    event :fail   do transitions to: :failed,      from: %i[pending started] end
    event :cancel do transitions to: :cancelled,   from: %i[started passed] end
