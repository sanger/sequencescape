class Request::ChangeDecision
  ...
  attr_accessor :request_state_check_box
  attr_accessor :asset_qc_state_check_box
  attr_accessor :billing_state_check_box

  def checkboxes
    [ self.request_state_check_box, self.asset_qc_state_check_box, self.billing_state_check_box ]
  end
  ...
  attr_accessor :state
  validates_each(:state, :unless => :state_absent?) do |record, attr, value|
    record.errors.add(:state, 'is not valid') unless record.valid_states_for_select.include?(value)
  end
  validates_presence_of :state, :unless => :state_absent?
...
#    record.errors.add(:state, 'cannot be same as current state') if value == record.request.state
#  end
  ...
  def perform_decision_change!
    begin
      ActiveRecord::Base.transaction do 
        perform_decision_change_request_state! unless state_absent?
