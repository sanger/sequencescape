...
class SubmissionsController < ApplicationController
  ...
  def index
    @building = Submission.building(:order => "created_at DESC", :conditions => { :user => current_user})
    @pending = Submission.pending(:order => "created_at DESC", :conditions => { :user => current_user})
    @ready = Submission.ready(:order => "updated_at DESC", :limit => 10, :conditions => { :user => current_user})
  end
