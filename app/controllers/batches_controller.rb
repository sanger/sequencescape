...
class BatchesController < ApplicationController # rubocop:todo Metrics/ClassLength
  ...
  # the batch. Ultima and Ultima UG200 sample sheets are allowed to be
  # downloaded without authentication. Otherwise, the user must be logged in.
  #
  # @return [Boolean] true if download is allowed, false otherwise
  def allow_sample_sheet_download?
    return true if defined?(UltimaUG200SequencingPipeline) && @batch.pipeline.is_a?(UltimaUG200SequencingPipeline)
  ...
  def generate_sample_sheet
    ...
    # Check whether the name UltimaUG200SequencingPipeline is defined.
    # Check before UltimaSequencingPipeline in case inherited.
    if defined?(UltimaUG200SequencingPipeline) && @batch.pipeline.is_a?(UltimaUG200SequencingPipeline)
      return generate_ultima_ug200_sample_sheet
    end
