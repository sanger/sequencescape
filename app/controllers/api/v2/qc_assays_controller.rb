    ...
    class QcAssaysController < JSONAPI::ResourceController
      def create
        ...
        if @qc_result_factory.valid?
          ...
          render json: JSONAPI::ResourceSerializer.new(QcAssayResource).serialize_to_hash(QcAssayResource.new(
                                                                                            @qc_result_factory.qc_assay, nil
                                                                                          )), status: :created
        else
      ...
      def qc_assay_params
        params.require(:data).require(:attributes).permit(:lot_number,
                                                          qc_results: %i[barcode uuid well_location key value units cv
