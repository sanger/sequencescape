...
module Aker
  module Factories
    ...
    class Job
      ...
      ATTRIBUTES = %i[job_id work_order_id job_url product_name process_name process_uuid modules product_version product_uuid project_uuid project_name cost_code container materials comment desired_date data_release_uuid].freeze
      DEFAULT_ATTRIBUTES = { materials: {} }.freeze

      attr_accessor(*ATTRIBUTES)
      attr_reader :aker_job_id, :model

      validates_presence_of :aker_job_id, :data_release_uuid, :materials, :job_url
      ...
      def create
        return unless valid?
        @model = Aker::Job.create(aker_job_id: aker_job_id, samples: collect_materials, job_url: job_url)
      end
