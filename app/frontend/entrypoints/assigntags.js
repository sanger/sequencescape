...
function highlightDuplicates() {
  ...
  taggers.each(function (index) {
    ...
    if (duplicates[chosenTagIndex]) duplicates[chosenTagIndex].push(index);
    else duplicates[chosenTagIndex] = [index];
  });
  // Highlight those rows which have duplicate tags
  taggers.each(function (_index) {
  ...
  if (duplicatesPresent) $("#stage_button").attr("disabled", true);
  else $("#stage_button").attr("disabled", false);
}

$("select.tagchoice").change(function () {
  var beginningOnTag = taggers.index($(this)) + 1;
