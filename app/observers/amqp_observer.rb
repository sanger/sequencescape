    ...
    class MostRecentBuffer
      ...
      def each(&block)
        @updated.group_by(&:first).each do |model, pairs|
          ...
          scope = if model.respond_to?(:including_associations_for_json)
                    model.unscoped.including_associations_for_json
                  else
                    model.unscoped
                  end
          pairs.map(&:last).in_groups_of(configatron.amqp.burst_size).each { |group| scope.find(group.compact).map(&block) }
        end
        @deleted.each(&block)
      end
...
class ActiveRecord::Base
  class << self
    ...
    alias_method(:transaction_without_amqp, :transaction)
    alias_method(:transaction, :transaction_with_amqp)
  end
  def routing_key;
    nil;
  end
