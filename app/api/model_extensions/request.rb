module ModelExtensions::Request
  def self.included(base)
    base.class_eval do
      named_scope :include_source_asset, :include => { :asset        => [ :uuid_object, :barcode_prefix, :scanned_into_lab_event, { :sample => :uuid_object } ] }
      named_scope :include_target_asset, :include => { :target_asset => [ :uuid_object, :barcode_prefix, { :sample => :uuid_object } ] }
