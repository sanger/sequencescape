...
(function ($, undefined) {
    ...
    $('select.tagchoice').change(function () {
        ...
        {
            var chosenTagIndex = $(this).prop("selectedIndex");
            var resetTagsWhenRunOut = ($("table#tag-assignment[data-fulfill-everything-automatically-with-tags=true]").length > 0);
            var $tagSelectsToUpdate = taggers.slice(beginningOnTag);
            var maxNumberOfTags = $tagSelectsToUpdate.first().find("option").length-1;
        ...
        if ($("table#tag-assignment[data-disable-checks=true]").length===0) {
          highlightDuplicates();
        }
    });
})(jQuery);
