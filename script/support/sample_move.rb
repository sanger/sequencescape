...
def new_move_samples(sample_names,study_from_id,study_to_id,user_login,rt_ticket,mode)
  ActiveRecord::Base.transaction do 
    fluidigm_plates = []; lane_ids = []; pb_tube_ids = []; movable_classes = ['Well','SampleTube','LibraryTube'], sample_manifests = []
    ...
    if mode == 'test'
      ...
      if !sample_manifests.compact.empty?
        ...
        sample_manifests.uniq.each do |sm|
          ...
          assets.map(&:receptacles).flatten.each {|tube| tube.messengers.first.resend} if assets.first.is_a?(SampleTube)
