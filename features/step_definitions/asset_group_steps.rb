...
Given /^the sample tube "([^\"]+)" is in the asset group "([^\"]+)"$/ do |tube_name, group_name|
  asset_group = AssetGroup.find_by_name(group_name) or raise StandardError, "Could not find asset group #{group_name.inspect}"
  sample_tube = SampleTube.find_by_name(tube_name) or raise StandardError, "Could not find sample tube #{tube_name.inspect}"
...
Then /^the assets in the asset group "([^\"]+)" should only be in that group$/ do |name|
  asset_group = AssetGroup.find_by_name(name) or raise StandardError, "Cannot find the asset group #{name.inspect}"
