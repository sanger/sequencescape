...
Given /^the sample manifest with ID (\d+) is for (\d+) preextracted plates?$/ do |id, count|
  manifest = SampleManifest.find(id)
  manifest.update_attributes!(:asset_type => 'pre_extracted_plate', :count => count.to_i)
end
