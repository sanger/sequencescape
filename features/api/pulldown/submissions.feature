...
Feature: Creating submissions for pulldown
  ...
  Scenario: A submission should error if you set an incorrect request option on construction
    ...
    And the JSON should be:
      ...
      {
        "content": {
          "request_options.read_length": ["is '22' should be 50, 75, 100, or 150"]
        }
      }
      """
  ...
  Scenario: A submission for a pulldown pipeline that uses bait libraries
    ...
    Then all "Pulldown ISC" requests should have the following details:
      | fragment_size_required_from | 100                 |
      | fragment_size_required_to   | 400                 |
      | bait_library.name           | Human all exon 50MB |

  @create
  Scenario: Attempting to set the fragment sizes to anything other than the default values should use the specified values
    Given the UUID for the submission template "Pulldown ISC - HiSeq paired end sequencing" is "00000000-1111-2222-3333-444444444444"
