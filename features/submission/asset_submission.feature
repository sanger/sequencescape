...
Feature: Submitting an asset directly for a request
  ...
  Scenario Outline: Project does not have any quota
    Given the library tube "library tube 1" has been involved in a "<sequencing type>" request within the study "Testing submission of an asset" for the project "Testing the submission of an asset"

    Given project "Testing the submission of an asset" has enforced quotas
    And the project "Testing the submission of an asset" has no "<sequencing type>" quota
    ...
    And I select "<read length>" from "Read length" for the "<sequencing type>" request type
    And I press "Create"
    Then I should not see "Created request"
    And I should see "Quotas are being enforced but have not been setup"
  ...
  Scenario Outline: Project does not have enough quota
    Given the library tube "library tube 1" has been involved in a "<sequencing type>" request within the study "Testing submission of an asset" for the project "Testing the submission of an asset"

    Given project "Testing the submission of an asset" has enforced quotas
    And the project "Testing the submission of an asset" has a "<sequencing type>" quota of 5
    ...
    And I fill in "Fragment size required (to)" with "200" for the "<sequencing type>" request type
    And I select "<read length>" from "Read length" for the "<sequencing type>" request type
    And I fill in "Number of requests to generate" with "10"
    And I press "Create"
    Then I should not see "Created request"
    And I should see "Insufficient quota"
