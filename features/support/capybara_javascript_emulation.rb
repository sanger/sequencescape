...
# This file is needed because the rack test driver is not able to click post/put/delete links bei default.
# In order to work with newer capybara versions, this file has been copied from the current cucumber-rails gem.
# https://github.com/cucumber/cucumber-rails/blob/master/lib/cucumber/rails/capybara/javascript_emulation.rb
  ...
  module Rails
    module Capybara
      module JavascriptEmulation
        ...
        def csrf?
          [csrf_param_node, csrf_token_node].all?(&:present?)
        end
        ...
        def emulated_method
          if ::Rails.version.to_f >= 3.0
            ...
            element_node['onclick'][/m\.setAttribute\('value', '([^']*)'\)/, 1] or element_node['onclick'][/f\.method = '([^']+)'/m, 1]
          end
        end
