...
Given(/^that there is a "([^"]*)" pipeline$/) { |pipeline_name| @pipeline = Pipeline.find_by(name: pipeline_name) }
...
Then(/^we see the requests in the inbox$/) do
  with_scope('#pipeline_inbox') do
    @pipeline.requests.map(&:asset).map(&:labware_name).each { |asset_name| assert page.has_content?(asset_name) }
  end
end
