...
Given(/^Pipeline "([^"]*)" and a setup for submitted at$/) do |name|
  ...
  request =
    FactoryBot.create :request_with_submission, request_type:, asset:, request_metadata: metadata
  request.asset.plate = FactoryBot.create(:plate) if request.asset.is_a?(Well) && request.asset.plate.nil?
