...
module RecordLoader
  ...
  class RequestTypeLoader < ApplicationRecordLoader
    ...
    def create_or_update!(key, options)
      ...
      creation_options['acceptable_plate_purposes'] = PlatePurpose.where(name: acceptable_plate_purpose_names) if acceptable_plate_purpose_names
      RequestType.create_with(creation_options)
