...
module SequencingQcPipeline
  ...
  def qc_pending_manual_batches
    ...
    Batch.where([
      "qc_state = ? AND qc_pipeline_id = ? AND pipeline_id in (?)", "qc_manual", self.id, cluster_formation_pipeline_id
  ...
  def qc_in_progress_auto_batches
    ...
    Batch.most_recent(20).where(["qc_state = ? OR qc_state = ? AND qc_pipeline_id = ? AND pipeline_id in (?)", "qc_submitted", "qc_criteria_received", self.id, cluster_formation_pipeline_id
      ]).includes(:user)
  end

  def qc_submitted_to_qc_batches
    Batch.most_recent(20).where(["qc_state = ?  AND qc_pipeline_id = ? AND pipeline_id in (?)", "qc_submitted", self.id, cluster_formation_pipeline_id
      ]).includes(:user)
  end
  ...
  def qc_in_progress_manual_batches
    ...
    Batch.most_recent(20).where([
      "qc_state = ? AND qc_pipeline_id = ? AND pipeline_id in (?)", "qc_manual_in_progress", self.id, cluster_formation_pipeline_id
    ]).includes(:user)
  end

  def qc_completed_auto_batches
    # {:state => "released", :qc_state => "qc_manual", :qc_pipeline_id => self.next_pipeline_id},
    Batch.most_recent(20).where(["qc_state = ? AND qc_pipeline_id = ? AND pipeline_id in (?)", "qc_manual", self.next_pipeline_id, cluster_formation_pipeline_id
      ]).includes(:user)
  end
