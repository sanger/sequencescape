...
PacBioSamplePrepPipeline
  .create!(name: 'PacBio Tagged Library Prep') do |pipeline|
    pipeline.sorter = 14
    pipeline.active = true

    pipeline.request_types <<
      RequestType.create!(key: 'pacbio_tagged_library_prep', name: 'PacBio Tagged Library Prep') do |request_type|
...
PacBioSequencingPipeline.find_by(name: 'PacBio Sequencing').request_types <<
  RequestType.create!(key: 'pacbio_multiplexed_sequencing', name: 'PacBio Multiplexed Sequencing') do |request_type|
    request_type.initial_state = 'pending'
