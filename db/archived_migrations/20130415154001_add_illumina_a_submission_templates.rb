...
class AddIlluminaASubmissionTemplates < ActiveRecord::Migration
  ...
  def self.new_templates
    [
      {:middle_name => 'HTP ISC', :middle_request_types => [ [shared], [isc] ] }
    ]
  end
    ...
    [true,false].each do |cherrypick|
      sequencing_requests.each do |sequencing_request|
        ...
        yield({
          ...
          :submission_parameters => {:request_type_ids_list => request_type_ids, :order_role_id => Order::OrderRole.find_or_create_by(role:'ILA ISC').id}.merge(sequencing_request[:submission_parameters]),
  ...
  def self.isc
    RequestType.find_by_key('illumina_a_isc').id
  end

  def self.sequencing_requests
    [
      {:name=>'HiSeq 2500 Paired end sequencing', :key=>["illumina_a_hiseq_2500_paired_end_sequencing","hiseq_2500_paired_end_sequencing"], :submission_parameters => {:workflow_id=>1, :info_differential=>1, :input_field_infos=>Hiseq2500Helper.other({:sub_params=>:ill_b})}},
      {:name=>'HiSeq 2500 Single end sequencing', :key=>["illumina_a_hiseq_2500_single_end_sequencing","hiseq_2500_single_end_sequencing"], :submission_parameters => {:workflow_id=>1, :info_differential=>1, :input_field_infos=>Hiseq2500Helper.other({:sub_params=>:ill_b_single})}},
      ...
      {:name=>'HiSeq Single ended sequencing', :key=>["illumina_a_single_ended_hi_seq_sequencing","single_ended_hi_seq_sequencing"], :submission_parameters => {:workflow_id=>1, :input_field_infos=>Hiseq2500Helper.other({:sub_params=>:ill_b_single})}},
