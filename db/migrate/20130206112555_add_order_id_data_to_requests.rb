class AddOrderIdDataToRequests < ActiveRecord::Migration
  ...
  def self.up
    Request.find_in_batches(:include=>{:submission=>:orders}, :conditions => ['submission_id IS NOT NULL']) do |batch|
  ...
  def self.down
    Request.all.find_in_batches(:conditions => ['order_id IS NOT NULL']) do |batch|
      ActiveRecord::Base.transaction do
        say "Processing requests #{batch.first.id} to #{batch.last.id}"
        batch.each do |request|
