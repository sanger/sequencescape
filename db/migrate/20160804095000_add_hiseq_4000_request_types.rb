class AddHiseq4000RequestTypes < ActiveRecord::Migration
  ...
  def self.down
    ActiveRecord::Base.transaction do
      ['htp', 'c'].each do |pipeline|
        RequestType.find_by_key("illumina_#{pipeline}_hiseq_4000_paired_end_sequencing").destroy
        RequestType.find_by_key("illumina_#{pipeline}_hiseq_4000_single_end_sequencing").destroy
      end
    end
  end
end
