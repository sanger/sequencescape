...
class UpdateEventsEventableTypes < ActiveRecord::Migration[5.1]
  def up
    ...
    Event::SequenomLoading.where(family: 'update_fluidigm_plate')
                          .update_all(eventful_type: 'Labware')
    Event::SequenomLoading.where(family: %w[update_gender_markers update_sequenom_count])
    ...
    Event.where(eventful_type: 'Asset')
