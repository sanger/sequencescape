...
class UpdateIdentifiersIdentifiableType < ActiveRecord::Migration[5.1]
  def up
    Identifier.where(external_type: 'Snp::DnaWell')
              .update_all(identifiable_type: 'Receptacle')
    Identifier.where(external_type: 'Snp::DnaPlate')
              .update_all(identifiable_type: 'Labware')
  end
