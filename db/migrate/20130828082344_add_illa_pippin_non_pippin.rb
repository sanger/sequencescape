    ...
    [true,false].each do |cherrypick|
      sequencing_requests.each do |sequencing_request|
        ...
        yield({
          ...
          :submission_parameters => {:request_type_ids_list => request_type_ids, :order_role_id => Order::OrderRole.find_or_create_by(role:'ILA').id}.merge(sequencing_request[:submission_parameters]),
