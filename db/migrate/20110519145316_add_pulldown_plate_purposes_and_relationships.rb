class AddPulldownPlatePurposesAndRelationships < ActiveRecord::Migration
  def self.up
    ActiveRecord::Base.transaction do
      Pipeline::Pulldown::PULLDOWN_PLATE_PURPOSE_FLOWS.each do |flow|
      ...
      Pipeline::Pulldown::PULLDOWN_PLATE_PURPOSE_LEADING_TO_QC_PLATES.each do |name|
  ...
  def self.down
    ActiveRecord::Base.transaction do
      PlatePurpose.destroy_all([
        'name IN (?)',
        Pipeline::Pulldown::PULLDOWN_PLATE_PURPOSE_FLOWS.flatten + [ 'Pulldown QC plate' ]
      ])
    end
  end
end
