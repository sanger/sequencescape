...
class WorkCompletionsTest < ActionDispatch::PerformanceTest
  ...
  def setup
    ...
    product = create :billing_product
    ...
    thirds = [tube_1_2, tube_3, tube_4].map do |start|
      ...
      create :transfer_request, asset: start, target_asset: second, submission_id: nil
      third = create :new_stock_multiplexed_library_tube#, parents: [second]
      create :transfer_request, asset: second, target_asset: third, submission_id: nil
      third
    end

    @target_tube = create :multiplexed_library_tube, parents: thirds
    thirds.each do |third|
      create :transfer_request, asset: third, target_asset: @target_tube, submission_id: nil
    end
  end
