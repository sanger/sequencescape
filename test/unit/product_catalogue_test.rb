...
class ProductCatalogueTest < ActiveSupport::TestCase
  context 'A product catalogue' do
    ...
    should_validate_presence_of :name, :selection_behaviour

    context 'with single product behaviour' do
      setup do
        @catalogue = Factory :single_product_catalogue
    ...
    context 'with invalid behaviour' do
      should 'reject non-existant behaviours'do
        assert_raise(ActiveRecord::RecordInvalid) do
          Factory :product_catalogue, :selection_behaviour => 'InvalidSelectionBehaviour'
        end
      end
    end
  ...
  context 'ProductCatalogue::construct!' do
    ...
    setup do
      ...
      @existing_product = Factory :product, :name => 'pre_existing'
