...
class QcReport::FileTest < ActiveSupport::TestCase
  ...
  context 'QcReport File' do
    context 'given a non-csv file' do
      setup do
        @file = fixture_file_upload("#{Rails.root.join('test/data/190_tube_sample_info.xls')}", 'text/csv')
    ...
    context 'given a non-compatible csv file' do
      setup do
        @file = fixture_file_upload("#{Rails.root.join('test/data/fluidigm.csv')}", 'text/csv')
    ...
    context 'given a file with no report' do
      setup do
        @file = fixture_file_upload("#{Rails.root.join('test/data/qc_report.csv')}", 'text/csv')
