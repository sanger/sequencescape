...
class RnachipTest < ActiveSupport::TestCase
  ...
  def setup
    ...
    @download = SampleManifestExcel::Download::Rnachip.new(sample_manifest, column_list, range_list)
    download.save('test.xlsx')
    @spreadsheet = Roo::Spreadsheet.open('test.xlsx')
  end

  test "should have correct columns_names" do
  	assert_equal 24, download.columns_names.count
  end
