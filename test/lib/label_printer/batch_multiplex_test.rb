...
class BatchMultiplexTest < ActiveSupport::TestCase
  ...
  attr_reader :tube_label, :prefix, :barcode1, :tube1, :label, :name
  ...
  def setup
    ...
    @barcode1 = '1111'
    @name = 'tube name'
    @tube1 = create :library_tube, barcode: barcode1, name: name
  ...
  test 'should return correct top_line value' do
    assert_equal "(p) #{name}", tube_label.top_line(tube1)
  end
