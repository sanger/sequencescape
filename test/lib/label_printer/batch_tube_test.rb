...
class BatchTubeTest < ActiveSupport::TestCase
	...
	attr_reader :batch_tube_label, :batch
	...
	context "stock" do
	    ...
	    library_tube_with_stock_tube.children << child
			...
			@batch_tube_label = LabelPrinter::Label::BatchTube.new(options)

			assert_equal 1, batch_tube_label.tubes.count
			tube = batch_tube_label.tubes.first
			assert_equal child.name, batch_tube_label.top_line(tube)
	 	end
	    ...
	    stock_library_tube.children << library_tube_with_stock_tube
			...
			assert_equal 1, batch_tube_label.tubes.count
			tube = batch_tube_label.tubes.first
			assert_equal request.target_asset.stock_asset.name, batch_tube_label.top_line(tube)
	...
	context "no stock" do
	    ...
	    batch = @pipeline.batches.create!
			...
			assert_equal 1, batch_tube_label.tubes.count
			tube = batch_tube_label.tubes.first
			assert_equal "(#{request.target_asset.tag}) #{request.target_asset.id}" , batch_tube_label.top_line(tube)
		...
		should "when not multiplexed should return the right tubes and top line" do
			...
			assert_equal 1, batch_tube_label.tubes.count
			tube = batch_tube_label.tubes.first
			assert_equal request.target_asset.tube_name, batch_tube_label.top_line(tube)
