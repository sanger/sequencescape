    ...
    context '#stage' do
      ...
      context 'should set descriptors on batch' do
        setup do
          ...
          request_data =
            @batch.requests.reload.map(&:id).each_with_object({}) { |element, result| result[element.to_s] = '1' }
          post :stage,
               params: {
        ...
        should 'change batch.lab_events by 1' do
          assert_equal 1,
                       Batch.find(@batch.id).lab_events.size - @batch_lab_events,
                       'Expected batch.lab_events to change by 1'
        end
