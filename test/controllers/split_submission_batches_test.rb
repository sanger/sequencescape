  ...
  context 'when I have a submission' do
    ...
    context 'which is single plexed' do
      ...
      context 'and I batch up the library creation requests seperately' do
        ...
        should 'before failing any sequencing requests' do
          assert_equal LibraryCreationRequest.first.id + @asset_count,
                       LibraryCreationRequest.first.next_requests.first.id
    ...
    context 'which is multiplexed' do
      ...
      should 'report correct groupings from the start' do
        assert_equal MultiplexedLibraryCreationRequest.first.id + 4,
                     MultiplexedLibraryCreationRequest.first.next_requests.first.id
        assert_equal MultiplexedLibraryCreationRequest.first.id + 4,
                     MultiplexedLibraryCreationRequest.all[2].next_requests.first.id
        assert_equal 5, MultiplexedLibraryCreationRequest.first.next_requests.size
        assert_equal MultiplexedLibraryCreationRequest.first.id + 8,
                     MultiplexedLibraryCreationRequest.first.next_requests.last.id
      end
    end
  end
end
