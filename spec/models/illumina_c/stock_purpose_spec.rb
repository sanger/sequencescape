# frozen_string_literal: true

require 'rails_helper'

# This behaviour is necessary because of limitations in generic lims
# We can switch to the PlatePurpose::Input class once we're in limber
describe IlluminaC::StockPurpose do
  let(:illumina_c_stock_purpose) { create :illumina_c_stock_purpose }
  let(:plate) { create :plate, purpose: illumina_c_stock_purpose, well_count: 2, well_factory: :untagged_well }

  describe '#state_of' do
    subject(:state_of) { illumina_c_stock_purpose.state_of(plate) }

    context 'with no requests' do
      it 'is pending' do
        expect(state_of).to eq('pending')
      end
    end

    context 'with two pending requests' do
      before do
        plate.wells.each do |well|
          create :request_library_creation, asset: well
        end
      end

      it 'is passed' do
        expect(state_of).to eq('passed')
      end
    end

    context 'with two failed requests' do
      before do
        plate.wells.each do |well|
          create :request_library_creation, asset: well, state: 'failed'
        end
      end

      it 'is failed' do
        expect(state_of).to eq('failed')
      end
    end

    context 'with two cancelled requests' do
      before do
        plate.wells.each do |well|
          create :request_library_creation, asset: well, state: 'cancelled'
        end
      end

      it 'is cancelled' do
        expect(state_of).to eq('cancelled')
      end
    end

    context 'with a mix of cancelled and failed requests' do
      before do
        create :request_library_creation, asset: plate.wells.first, state: 'cancelled'
        create :request_library_creation, asset: plate.wells.last, state: 'failed'
      end

      it 'is failed' do
        expect(state_of).to eq('failed')
      end
    end

    # Generic Lims assumes we only have one active submission at a time per well.
    # If we have multiple, it doesn't know what to do, so we prevent anything happening.
    # The branch where this test was written adds support for this functionality in
    # Limber, so we can kill this behaviour once we move Generic Lims across
    context 'with two active requests' do
      before do
        plate.wells.each do |well|
          create_list :request_library_creation, 2, asset: well
        end
      end

      it 'is pending' do
        expect(state_of).to eq('pending')
      end
    end

    context 'with one active and one cancelled requests' do
      before do
        plate.wells.each do |well|
          create :request_library_creation, asset: well
          create :request_library_creation, asset: well, state: 'cancelled'
        end
      end

      it 'is passed' do
        expect(state_of).to eq('passed')
      end
    end
  end
end
