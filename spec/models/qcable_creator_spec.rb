...
describe QcableCreator do
  ...
  it 'creates some qcables with a count' do
    qcable_creator = described_class.create(count: 10, user:, lot:)
    expect(qcable_creator.qcables.count).to eq(10)
  end

  it 'creates some qcables with a list of barcodes' do
  ...
  context 'with supplied plate barcode' do
    ...
    it 'creates qcables with single barcode' do
      qcable_creator = described_class.create(user:, lot:, supplied_barcode: plate_barcode)
