...
RSpec.describe StateChanger::TubeRack do
  ...
  context 'when the target state is "passed"' do
    ...
    context 'when transitioning to "passed" state for a rack with multiple tubes' do
      ...
      let(:requests) do
        [
          create(:request, target_asset: labware.tube_receptacles[0], state: request_state),
          create(:request, target_asset: labware.tube_receptacles[1], state: request_state),
          create(:request, target_asset: labware.tube_receptacles[2], state: request_state)
        ]
      end
      let(:transfer_requests) do
        [
          create(:transfer_request, target_asset: labware.tube_receptacles[0], state: transfer_request_state),
          create(:transfer_request, target_asset: labware.tube_receptacles[1], state: transfer_request_state),
          create(:transfer_request, target_asset: labware.tube_receptacles[2], state: transfer_request_state)
        ]
      end
      ...
      xit 'updates the tube to "passed" for receptacles with "started" requests', :aggregate_failures do
        labware.tube_receptacles.each do |tube_receptacle|
          expect(tube_receptacle.transfer_requests_as_target.first.reload.state).to eq('passed')
        end
      end
    end
  end
