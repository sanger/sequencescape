...
RSpec.describe SampleManifestUploader, type: :model, sample_manifest_excel: true do
  ...
  it 'will not be valid without a filename' do
    expect(SampleManifestUploader.new(nil, SampleManifestExcel.configuration, user)).to_not be_valid
  end
  ...
  it 'will not be valid without some configuration' do
    ...
    expect(SampleManifestUploader.new(test_file, nil, user)).to_not be_valid
  end
  ...
  it 'will not be valid without a tag group' do
    ...
    download = build(:test_download, columns: SampleManifestExcel.configuration.columns.tube_library_with_tag_sequences.dup)
    download.save(test_file)
    expect(SampleManifestUploader.new(test_file, SampleManifestExcel.configuration, user)).to_not be_valid
  ...
  it 'will not be valid without a user' do
    download = build(:test_download, columns: SampleManifestExcel.configuration.columns.tube_library_with_tag_sequences.dup)
    download.save(test_file)
    expect(SampleManifestUploader.new(test_file, SampleManifestExcel.configuration, nil)).to_not be_valid
  end
