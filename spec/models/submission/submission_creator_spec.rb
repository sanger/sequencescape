    ...
    context 'a full template' do
      ...
      it 'finds the appropriate order fields' do
        ...
        expect(creator.order_fields).to include(
          FieldInfo.new(display_name: 'Fragment size required (from)', key: :fragment_size_required_from,
                        kind: 'Numeric')
        )
        expect(creator.order_fields).to include(
        ...
        expect(creator.order_fields).to include(
          FieldInfo.new(default_value: 'Standard', display_name: 'Library type', key: :library_type,
                        kind: 'Selection', selection: ['Standard'])
        )
        expect(creator.order_fields).to include(
          FieldInfo.new(display_name: 'PCR Cycles', key: :pcr_cycles, kind: 'Numeric')
        )
        expect(creator.order_fields).to include(
          FieldInfo.new(default_value: 54, display_name: 'Read length', key: :read_length, kind: 'Selection',
