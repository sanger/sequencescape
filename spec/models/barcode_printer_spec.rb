...
describe BarcodePrinter do
 ...
 create(:barcode_printer, name: 'test_printer', barcode_printer_type: create(:plate_barcode_printer_type)) end
  let!(:barcode_printer_type_plate_384) do
 BarcodePrinterType.find_by(name: '384 Well Plate') || create(:barcode_printer_type, name: '384 Well Plate') end
  ...
  it 'registers in PMB after create' do
    ...
    allow(RestClient).to receive(:get).with('http://localhost:9292/v2/printers?filter[name]=test_printer', 
content_type: 'application/vnd.api+json', accept: 'application/vnd.api+json').and_return('{"data":[]}')
    allow(RestClient).to receive(:post).with('http://localhost:9292/v2/printers', 
{ 'data' => { 'attributes' => { 'name' => 'test_printer', printer_type: 'squix' } } }.to_json, content_type: 'application/vnd.api+json', accept: 'application/vnd.api+json').and_return(201)
