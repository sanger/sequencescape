...
RSpec.describe 'samples/index.html.erb', type: :view do
  ...
  context 'when rendering the index view' do
    ...
    context 'when the user has withdrawn consent' do
      before do
        samples[0].update(consent_withdrawn: true,
                          user_id_of_consent_withdrawn: current_user.id, date_of_consent_withdrawn: time)

        samples[2].update(consent_withdrawn: true,
                          user_id_of_consent_withdrawn: current_user.id, date_of_consent_withdrawn: time2)
      ...
      it 'renders the withdrawn message for each sample' do
        ...
        regexp2 = Regexp.new("Patient consent has been withdrawn for this sample.*by user.*#{current_user.login}.*at .*#{time2.to_formatted_s(:db)}.*", Regexp::MULTILINE)
        render
    ...
    context 'when the user has no withdrawn consent' do
      before do
        samples[0].update(consent_withdrawn: false,
                          user_id_of_consent_withdrawn: current_user.id, date_of_consent_withdrawn: time)

        samples[2].update(consent_withdrawn: false,
                          user_id_of_consent_withdrawn: current_user.id, date_of_consent_withdrawn: time2)
