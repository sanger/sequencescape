...
RSpec.describe SampleManifestExcel::Validation, type: :model, sample_manifest_excel: true do
  ...
  it 'is not valid without options' do
    expect(SampleManifestExcel::Validation.new).to_not be_valid
  end

  it 'is comparable' do
    expect(SampleManifestExcel::Validation.new(options: options)).to eq(SampleManifestExcel::Validation.new(options: options))
    expect(SampleManifestExcel::Validation.new(options: options.except(:formula1))).to_not eq(SampleManifestExcel::Validation.new(options: options))
  end

  context 'without range name' do
    let(:validation) { SampleManifestExcel::Validation.new(options: options) }
  ...
  context 'with range name' do
    let(:validation) { SampleManifestExcel::Validation.new(options: options, range_name: :a_range) }
