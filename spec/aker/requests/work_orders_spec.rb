...
RSpec.describe Aker::WorkOrdersController, type: :request, aker: true do
  ...
  scenario 'complete a work order' do
    allow(RestClient::Request).to receive(:execute).with(method: :post, url: "#{url}/complete", params: {work_order: {id: work_order.aker_id, comment: "Complete it"}}, headers: {content_type: :json}, proxy: nil).and_return(RestClient::Response.create({work_order: {id: work_order.aker_id, comment: "Complete it"}}.to_json, Net::HTTPResponse.new('1.1',200,''), request))
  ...
  scenario 'cancel a work order' do
    allow(RestClient::Request).to receive(:execute).with(method: :post, url: "#{url}/cancel", params: {work_order: {id: work_order.aker_id, comment: "Cancel it"}}, headers: {content_type: :json}, proxy: nil).and_return(RestClient::Response.create({work_order: {id: work_order.aker_id, comment: "Cancel it"}}.to_json, Net::HTTPResponse.new('1.1',200,''), request))
