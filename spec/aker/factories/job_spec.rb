...
RSpec.describe Aker::Factories::Job, type: :model, aker: true do
  ...
  it 'is valid with aker job id, data_release_uuid and materials' do
    job = described_class.new(params)
  ...
  it 'must have an aker job id which is a number' do
    job = described_class.new(params.except(:job_id))
    expect(job).not_to be_valid
  end

  it 'must have some materials' do
    job = described_class.new(params.except(:materials))
    expect(job).not_to be_valid

    job = described_class.new(params.merge(materials: []))
    expect(job).not_to be_valid
  end

  it 'is not valid unless all of the materials are valid' do
    job = described_class.new(params.merge(materials: params[:materials].push(params[:materials].first.except(:_id))))
    expect(job).not_to be_valid
  end
  ...
  it '#create persists the job if it is valid' do
    ...
    expect(described_class.create(params.except(:materials))).to be_nil
  end
  ...
  it '#create will fail if the materials exist but the containers has changed' do
    ...
    expect do
      described_class.create(params.merge(job_uuid: SecureRandom.uuid, container: wrong_container_params))
  ...
  it 'creating a job with existing materials will add the study to the list of studies for the sample' do
    ...
    job = described_class.new(params).create
    job = Aker::Job.find_by(aker_job_id: job.aker_job_id)
    expect(job).to be_present
  ...
  it 'is not valid unless there is a data release uuid (study)' do
    job = described_class.new(params.except(:data_release_uuid))
    expect(job).not_to be_valid
  end

  it 'is not valid unless the study exists' do
    job = described_class.new(params.merge(data_release_uuid: SecureRandom.uuid))
    expect(job).not_to be_valid
  end

  it 'is not valid unless the study is active' do
    study = create(:study_for_study_list_inactive)
    job = described_class.new(params.merge(data_release_uuid: study.uuid))
    expect(job).not_to be_valid
  end
  ...
  it 'ignores extra container params' do
    ...
    job = described_class.create(params.merge(container: extra_container_params))
    job = Aker::Job.find_by(aker_job_id: job.aker_job_id)
    expect(job).to be_present
  end
end
