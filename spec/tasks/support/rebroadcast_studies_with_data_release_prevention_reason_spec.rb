...
describe 'support:rebroadcast_with_data_release_prevention_reason', type: :task do
  ...
  context 'when there are no studies with a data_release_prevention_reason' do
    ...
    it 'does not rebroadcast any study' do
      expect_any_instance_of(Study).not_to receive(:rebroadcast)
      task.invoke
    end
  end
  ...
  context 'when studies have a data_release_prevention_reason' do
    ...
    it 'calls rebroadcast on the study' do
      expect_any_instance_of(Study).to receive(:rebroadcast).once
      task.invoke
    end
  ...
  context 'when only some studies have a data_release_prevention_reason' do
    ...
    it 'calls rebroadcast exactly once per matching study' do
      rebroadcast_count = 0
      allow_any_instance_of(Study).to receive(:rebroadcast) { rebroadcast_count += 1 }
      task.invoke
      expect(rebroadcast_count).to eq(2)
    end
  end
  ...
  context 'when DRY_RUN=true' do
    ...
    it 'does not call rebroadcast' do
      expect_any_instance_of(Study).not_to receive(:rebroadcast)
      task.invoke
    end
