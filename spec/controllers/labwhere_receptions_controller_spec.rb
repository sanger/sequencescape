    ...
    shared_examples 'a reception' do
      setup do
        expect(LabWhereClient::Scan).to receive(:create).with(
          location_barcode: location_barcode, user_code: SBCF::SangerBarcode.from_human(user.barcode).machine_barcode.to_s, labware_barcodes: [plate.ean13_barcode, plate_2.ean13_barcode, sample_tube.ean13_barcode]
