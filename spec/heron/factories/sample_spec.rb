  ...
  describe '#create' do
    ...
    context 'when the factory is valid' do
      ...
      context 'when providing sample_uuid' do
        ...
        it 'returns the sample with uuid specified' do
          factory = described_class.new(study:, sample_uuid: sample.uuid)
          expect(factory.create).to eq(sample)
        end

        it 'is invalid if providing any other extra attributes' do
          factory = described_class.new(study:, sample_uuid: sample.uuid, sample_id: '1234')
          expect(factory).not_to be_valid
        end

        it 'is valid if providing any other attributes not sample related' do
