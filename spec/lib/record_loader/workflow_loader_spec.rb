...
require 'record_loader/workflow_loader'

# This file was initially generated via `rails g record_loader`
RSpec.describe RecordLoader::WorkflowLoader, type: :model, loader: true do
  ...
  let(:test_directory) { Rails.root.join('spec/data/record_loader/workflows') }

  context 'with two_entry_example selected' do
    let(:selected_files) { 'two_entry_example' }

    it 'creates two records' do
      expect { record_loader.create! }.to change(Workflow, :count).by(2)
    end
    ...
    it 'is idempotent' do
      ...
      expect { record_loader.create! }.not_to change(Workflow, :count)
    end
