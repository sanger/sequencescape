...
RSpec.describe Accession::Response, type: :model, accession: true do
  ...
  it 'is successful if the status code is in the correct range' do
    ...
    expect(Accession::Response.new(MockAccession::Response.new(400, ''))).not_to be_success
    expect(Accession::Response.new(MockAccession::Response.new(500, ''))).not_to be_success
  end

  it 'is a failure if the status code is in the correct range' do
    expect(Accession::Response.new(MockAccession::Response.new(200, ''))).not_to be_failure
    expect(Accession::Response.new(MockAccession::Response.new(300, ''))).not_to be_failure
