...
describe 'Labwhere reception', :js do
  ...
  it 'user can scan plates into the reception' do
    ...
    within('#new_labwhere_reception') do
      ...
      fill_in('asset_scan', with: 'TEST222').send_keys(:return)
      expect(page).to have_text('TEST222', count: 1)
      expect(page).to have_text 'Scanned: 3'
    ...
    expect(page).to have_text 'Locations updated!'
    expect(page).to have_text plate.human_barcode
    expect(page).to have_text plate.purpose.name
  ...
  it 'displays the correct labwhere error messages' do
    ...
    fill_in('asset_scan', with: plate.human_barcode).send_keys(:return)
    click_on 'Update locations'
    expect(page).to have_text 'LabWhere User does not exist'
    expect(page).to have_no_text plate.human_barcode
    expect(page).to have_no_text plate.purpose.name
