...
describe 'Create printers' do
  ...
  it 'can create a new printer' do
    ...
    allow(RestClient).to receive(:get).with('http://localhost:9292/v2/printers?filter[name]=yetanotherprinter', 
...
{ 'data' => { 'attributes' => { 'name' => 'yetanotherprinter', 'printer_type' => 'squix' } } }.to_json, content_type: 'application/vnd.api+json', accept: 'application/vnd.api+json').and_return(201)
    login_user user
    visit admin_path
