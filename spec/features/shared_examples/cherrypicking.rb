        ...
        (1..current_expected_plates.size).each do |pick_number_index|
          ...
          step "Download pick file for destination plate #{destination_barcode} pick number #{pick_number_index}" do
            ...
            when 'Hamilton'
              ...
              generated_file = DownloadHelpers.downloaded_file("#{batch_id}_batch_#{destination_barcode}_#{pick_number_index}.csv", timeout: 60)
              # TODO: This #lines method sometimes return an empty string when it doesnt have enough time to generate the content.
              # We increased the timeout to 20 seconds, but could still be creating problems
